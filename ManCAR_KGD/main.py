...
def parse_global_args(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Whether to regenerate intermediate files and recompute reader-side Swing statistics",
    )
    return parser
...
def main():
    ...
    if not args.regenerate and os.path.exists(corpus_path):
        ...
        logging.info(
            "Reader-side Swing overlap statistics are loaded from cache. "
            "Use --regenerate 1 to recompute them."
        )
    ...
    if args.load > 0:
        if not args.load_model_path:
            raise ValueError(
                "--load_model_path must be provided when --load is set to 1."
            )
        model.load_model(args.load_model_path)
    logging.info("Valid Before Training: " + runner.print_res(data_dict["valid"])[0])
    logging.info("Test Before Training: " + runner.print_res(data_dict["test"])[0])
    if args.train > 0:
        runner.train(data_dict)

    # Evaluate final results
    # val_res, _ = runner.print_res(data_dict["valid"])
    test_res, test_result_dict = runner.print_res(data_dict["test"])
...
if __name__ == "__main__":
    ...
    if temp_args.gpu and temp_args.gpu != "":
        os.environ["CUDA_VISIBLE_DEVICES"] = str(temp_args.gpu)
        device = "cuda"
    else:
        device = "cpu"
