...
class BaseRunner(object):
    ...
    @staticmethod
    def evaluate_method(
        top_indices: np.ndarray, targets: np.ndarray, topk: list, metrics: list
    ) -> Dict[str, float]:
        """
        :param top_indices: (-1, max_topk) shape, batched top-k item ids
        ...
        """
        evaluations = dict()
        pos_matrix = top_indices == targets.reshape(-1, 1)
        for m in metrics:
    ...
    def train(self, data_dict: Dict[str, BaseModel.Dataset]):
        ...
        self.best_valid_result = valid_results[best_epoch]
        self.final_train_time = self.time[1] - self.time[0]
        logging.info(
            os.linesep
            + "Best Iter(valid)={:>5}\t valid=({}) [{:<.1f} s] ".format(
                self.best_epoch,
                utils.format_metric(self.best_valid_result),
                self.final_train_time,
            )
        )
        model.load_model()
    ...
    def evaluate(
        ...
        """
        predictions, item_ids = self.predict(dataset)
        return self.evaluate_method(predictions, item_ids, topks, metrics)
    ...
    def predict(self, dataset: BaseModel.Dataset) -> np.ndarray:
        ...
        with torch.no_grad():
            for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc="Predict"):
                ...
                out = dataset.model.inference(batch_gpu)
                prediction = out["prediction"]
                prediction[:, 0] = -torch.inf  # mask padding item

                _, batch_topk = torch.topk(prediction, k=max_topk, dim=1)
                topk_indices.append(batch_topk.cpu().numpy())
                item_ids.append(batch[ITEM_ID].cpu().numpy())

                del batch_gpu, out, prediction, batch_topk
                batch_idx += 1
                if batch_idx % 10 == 0:
                    gc.collect()
                    torch.cuda.empty_cache()

        return np.vstack(topk_indices), np.concatenate(item_ids)

    def print_res(self, dataset: BaseModel.Dataset) -> tuple:
        """
        Evaluate and format results for a dataset.
        :return: (res_str, result_dict)
        """
