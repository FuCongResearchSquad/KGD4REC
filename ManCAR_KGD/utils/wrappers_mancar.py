...
class PromptReaRecBertWrapper(PromptBertAutoRegressiveWrapper):
    ...
    def _get_reason_seed(self, hs, batch_size, prompt_len, step, trigger_pos=None):
        """Return the hidden state used as seed for the next reasoning token.

        Default: last token of the clean (non-noise) half of the batch.
        Subclasses may override to use a different position (e.g. trigger).
        """
        return hs[:batch_size, -1:, :]
    ...
    def forward(self, input_embs, input_lens, prompt_ids, item_embs, epoch, i2i_label=None, target_id=None, stage='infer', noise_factor=0.01, reason_step=None, trigger_pos=None):
        """
        ...
        trigger_pos : [B] optional, position index within the padded history
                      sequence.  Only used by subclasses that override
                      ``_get_reason_seed`` (e.g. TriggerSeedReaRecBertWrapper).
        ...
        for step in range(reason_step + 1):
            ...
            sup_h = self._get_reason_seed(hs, batch_size, prompt_len, step, trigger_pos)
            all_outputs.append(sup_h)

            seed_h = self.adpter_norm(sup_h, item_embs)
...
class PromptReaRecSupBertWrapper(PromptBertAutoRegressiveWrapper):
    ...
    def forward(self, input_embs, input_lens, prompt_ids, item_emb, noise_factor=0.01, reason_step=None):
        ...
        for step in range(reason_step + 1):
            pad_mask = self._prepare_padding_mask(input_lens, prompt_ids, step, device)
            attn_mask = self._prepare_attention_mask(batch_size * repeat_batch, seq_len, step, device, pad_mask).bool()
